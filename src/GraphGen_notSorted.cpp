...
void fill_up_edge_vector(Square &rec, std::vector<Edge> &edgeVector,
    ...
    if (!allowDuplicateEdges) {
        ...
        do {
            ...
            if (!throwAwayEdgesIndices.empty())
                generate_edges(std::ref(rec), std::ref(edgeVector), RMAT_a, RMAT_b, RMAT_c,
...
bool GraphGen_notSorted::GenerateGraph(const unsigned long long nEdges,
    ...
    if (USE_A_MUTEX_TO_CONTROL_WRITE_TO_FILE_INSTEAD_OF_CONCURRENT_QUEUES) {
        ...
        unsigned int nWorkerThreads =
            nCPUWorkerThreads < squares.size() ? nCPUWorkerThreads : squares.size();

        auto eachThreadGenEdgesUsingMutexFunc = [&](unsigned int puId) {
            std::vector<Edge> edgeVector;

            std::mt19937_64 gen(rseed + puId);
            std::uniform_int_distribution<> dis;

            for (unsigned int recIdx = puId; recIdx < squares.size(); recIdx += nWorkerThreads) {
                auto &rec = squares.at(recIdx);
                fill_up_edge_vector(std::ref(rec), std::ref(edgeVector), std::ref(dis),
                                    std::ref(gen), RMAT_a, RMAT_b, RMAT_c, allowEdgeToSelf,
