...
void EachThreadGeneratesEdges(std::vector<Square> &recs, std::vector<Edge> &edgesVec,
                              const double RMAT_a, const double RMAT_b, const double RMAT_c,
                              const bool allowEdgeToSelf, const bool allowDuplicateEdges,
    ...
    std::vector<unsigned long long>
        throwAwayEdgesIndices; // Indices for those edges that must be eliminated due to duplicates
                               // and/or having same destination and source index.

    // For all the rectangles we have to generate edge for.
    for (auto &rec : recs) {

        generate_edges(std::ref(rec), std::ref(edgesVec), RMAT_a, RMAT_b, RMAT_c, directedGraph,
                       allowEdgeToSelf, std::ref(dis), std::ref(gen),
        ...
        if (!allowDuplicateEdges) {
            ...
            do {
                ...
                if (!throwAwayEdgesIndices.empty()) {
                    ...
                    generate_edges(std::ref(rec), std::ref(edgesVec), RMAT_a, RMAT_b, RMAT_c,
                                   directedGraph, allowEdgeToSelf, std::ref(dis), std::ref(gen),
                                   std::ref(throwAwayEdgesIndices));
...
bool GraphGen_sorted::GenerateGraph(const unsigned long long nEdges,
                                    const unsigned long long nVertices, const double RMAT_a,
                                    double RMAT_b, double RMAT_c,
    ...
    while (!Eligible_RNG_Squares(squares, standardCapacity)) {
        // For sorted version, we break down all the rectangles each time there's a non-eligible
        // rectangle.
    ...
    if (USE_FUTURES_INSTEAD_OF_EXPLICIT_THREADS) {
        ...
        for (; recIdx < nCPUWorkerThreads && recIdx < rectagnleVecs.size(); ++recIdx)
            tasks_to_complete.push_back(std::async(
                std::launch::async, EachThreadGeneratesEdges, std::ref(rectagnleVecs.at(recIdx)),
                std::ref(threads_edges[recIdx]), RMAT_a, RMAT_b, RMAT_c, allowEdgeToSelf,
                allowDuplicateEdges, directedGraph));

        // If any jobs left, main thread has to wait for the first worker thread and write the
        // outcome before initiating any other worker thread.
        for (; recIdx < rectagnleVecs.size(); ++recIdx) {
            ...
            tasks_to_complete.push_back(std::async(
                std::launch::async, EachThreadGeneratesEdges, std::ref(rectagnleVecs.at(recIdx)),
                std::ref(threads_edges[recIdx % nCPUWorkerThreads]), RMAT_a, RMAT_b, RMAT_c,
                allowEdgeToSelf, allowDuplicateEdges, directedGraph));
        }
        ...
        for (; recIdx < nCPUWorkerThreads && recIdx < rectagnleVecs.size(); ++recIdx)
            threads.push_back(std::thread(EachThreadGeneratesEdges,
                                          ...
                                          std::ref(threads_edges[recIdx]), RMAT_a, RMAT_b, RMAT_c,
                                          allowEdgeToSelf, allowDuplicateEdges, directedGraph));
        ...
        for (; recIdx < rectagnleVecs.size(); ++recIdx) {
            ...
            threads.push_back(
                std::thread(EachThreadGeneratesEdges, std::ref(rectagnleVecs.at(recIdx)),
                            std::ref(threads_edges[recIdx % nCPUWorkerThreads]), RMAT_a, RMAT_b,
