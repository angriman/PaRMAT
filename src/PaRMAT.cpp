...
int main(int argc, char **argv) {
    ...
    std::string usage = "\tRequired command line arguments:\n\
    ...
    try {
        ...
        for (int iii = 1; iii < argc; ++iii) {
            if (!strcmp(argv[iii], "-nEdges") && iii != argc - 1 /*is not the last one*/)
                ...
                nCPUWorkerThreads =
                    determine_num_of_CPU_worker_thread(std::stoul(std::string(argv[iii + 1])));
                ...
                directedGraph = false;
            else if (!strcmp(argv[iii], "-rseed") && iii != argc - 1)
                randomSeed = std::stoi(argv[iii + 1]);
        }

        if (nVertices == 0 || nEdges == 0 || nEdges >= nVertices * nVertices)
            throw std::runtime_error(
                "Number of edges or number of vertices are not specified (correctly).");
        ...
        if (nCPUWorkerThreads
            == 0) // If number of concurrent threads haven't specified by the user,
            nCPUWorkerThreads = std::max(1, static_cast<int>(std::thread::hardware_concurrency())
                                                - 1); // try to manage their numbers automatically.
                                                      // If cannot determine, go single-threaded.

        // Print the info.
        std::cout << "Requested graph will have " << nEdges << " edges and " << nVertices
                  << " vertices."
                  << "\n"
                  << "Its a, b, and c parameters will be respectively " << a << ", " << b
                  << ", and " << c << "."
                  << "\n"
                  << "There can be up to " << nCPUWorkerThreads
                  << " worker CPU thread(s) making the graph."
                  << "\n"
                  << "The graph will" << (sorted ? " " : " NOT ") << "necessarily be sorted."
                  << "\n"
                  << "Up to about " << (RAM_usage * 100.0)
                  << " percent of RAM can be used by this program."
                  << "\n"
                  << "Specified graph may" << (allowEdgeToSelf ? " " : " NOT ")
                  << "contain edges that have same source and destination index."
                  << "\n"
                  << "Specified graph may" << (allowDuplicateEdges ? " " : " NOT ")
    ...
    try {
        ...
        auto fOutcome =
            sorted
                ? GraphGen_sorted::GenerateGraph(nEdges, nVertices, a, b, c, nCPUWorkerThreads,
                                                 outf, standardCapacity, allowEdgeToSelf,
                                                 allowDuplicateEdges, directedGraph)
                : GraphGen_notSorted::GenerateGraph(nEdges, nVertices, a, b, c, nCPUWorkerThreads,
                                                    outf, standardCapacity, allowEdgeToSelf,
                                                    allowDuplicateEdges, directedGraph, randomSeed);
